    ...
    @if ($logged_in_user->can($permission.'.delete'))
        <div style="min-width: 80px">
            <x-utils.form-button-icon
                :action="route($route.'.restore', ['post' => $model])"
                method="patch"
            ...
            <x-utils.delete-button
                :href="route($route.'.permanently-delete', ['post' => $model])"
                :text="__('Permanently Delete')" />
        </div>
    @endif
@else
    @if ($logged_in_user->can('admin.access.forms.show'))
    <x-utils.view-button :href="route($route.'.show', $model)" />
    @endif
    @if ($logged_in_user->can('admin.access.forms.edit'))
    <x-utils.edit-button :href="route($route.'.edit', $model)" />
    @endif
    @if ($logged_in_user->can('admin.access.forms.delete'))
    <x-utils.delete-button :href="route($route.'.destroy', $model)" />
    @endif

    @if ($logged_in_user->can('admin.access.form-submission.show') || $logged_in_user->can('admin.access.form-fields.show'))
    ...
    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg-light-primary fw-semibold w-200px" data-kt-menu="true" style="">
        ...
        <div class="menu-item px-3">
            ...
            <a href="{{ route($route.'.field', $model) }}" class="menu-link px-3">
                Fields
            </a>
            @endif
            @if ($logged_in_user->can('admin.access.form-submission.show'))
            <a href="{{ route($route.'.submission', $model) }}" class="menu-link px-3">
                Submission
            </a>
            @endif
        </div>
