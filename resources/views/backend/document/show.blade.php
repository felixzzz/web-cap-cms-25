...
@section('content')
    <x-backend.card>
        ...
        <x-slot name="headerActions">
            <x-utils.link class="card-header-action" :href="route('admin.category.index', $template)" :text="__('Back')" />
        </x-slot>
        ...
        <x-slot name="body">
            <table class="table table-hover">
                ...
                <tr>
                    <th>@lang('ALT Image')</th>
                    <td>ID : {{ $category->alt_image }} <br> EN : {{ $category->alt_image_en }}</td>
                </tr>
                </table>
        </x-slot>
