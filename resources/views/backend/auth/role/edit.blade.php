...
@section('content')
    <x-forms.patch :action="route('admin.role.update', $role)" class="card mb-5 mb-xxl-8">
        <x-backend.card>
            <x-slot name="header">
                @lang('Update Role')
            </x-slot>
            ...
            <x-slot name="headerActions">
                <x-utils.link class="btn btn-sm btn-light" :href="route('admin.role.index')" :text="__('Cancel')" />
            </x-slot>

            <x-slot name="body">
                <div x-data="{userType : '{{ $role->type }}'}">
                    @if($enable_user_type_non_admin)
                        <div class="row mb-8">
                            <label for="name" class="col-md-3 form-label">@lang('Type')</label>
                    ...
                    @else
                        <input name="type" type="hidden" value="{{ $model::TYPE_ADMIN }}" />
                    @endif
