...
@section('content')
    ...
    <form class="form d-flex flex-column flex-lg-row fv-plugins-bootstrap5 fv-plugins-framework" method="post"
        action="{{ route('admin.post.store', ['type' => $type['type']]) }}" enctype="multipart/form-data">
        @csrf
                                ...
                                @if ($type['type'] != 'managements')
                                    ...
                                    @if ($type['type'] != 'blog')
                                        <div class="d-flex gap-4">
                                            @if ($type['is_category'])
                                                <div class="flex-fill">
                                                    <x-forms.select label="Category" name="categories[]"
                                                        placeholder="Select an option" required="{{ $required }}"
                                                        multiple="1" text="Choose category that has been defined"
                                                        hidden="{{ $hidden }}">
                                    ...
                                    @if ($type['type'] == 'news')
                                        <div class="flex-fill">
                                            <div class="mb-8 fv-row fv-plugins-icon-container">
                                                ...
                                                <select name="post_type" class=" form-control form-select-solid mb-2"
                                                    id="page">
                                                    <option value=""></option>
                                    ...
                                    <div class="d-flex gap-4">
                                        <div class="flex-fill">
                                            <div class="mb-8 fv-row fv-plugins-icon-container">
                                                ...
                                                <select name="status" class=" form-control form-select-solid mb-2"
                                                    required="">
                                                    <option value="{{ $model::STATUS_PUBLISH }}">@lang('Publish')</option>
                                                    <option value="{{ $model::STATUS_SCHEDULE }}">@lang('Schedule')
                                                    </option>
                                    ...
                                    @if ($type['featured_image'] & ($type['type'] != 'products'))
                                        <x-forms.filepond-input name="featured_image" label="Featured Image" class=""
                                            required="0" src=""
                                            text="Set the post thumbnail image. Only *.png, *.jpg, *.webp and *.jpeg image files are accepted"
                                            hidden="{{ $hidden }}" />
                                        <x-forms.text-input name="alt_image" label="Alt Image (ID)"
                                            required="{{ $required }}" placeholder="" text="" />
                                        <x-forms.text-input name="alt_image_en" label="Alt Image (EN)"
                                            required="{{ $required }}" placeholder="" text="" />
                                    @endif
                                @endif
                                @if ($components)
                                    <div class="card card-flush p-3 mt-8">
                                        <div class="card-body p-3">
                                            <div id="app">
                                                @if ($type['type'] == 'managements')
                                                    <x-forms.text-input name="title" label="Name"
                                                        required="{{ $required }}" placeholder="" text="" />
                                                    <div class="d-flex gap-4">
                                                        @if ($type['is_category'])
                                                            <div class="flex-fill">
                                                                <x-forms.select label="Category" name="categories[]"
                                                                    ...
                                                                    required="{{ $required }}" multiple="0"
                                                                    text="Choose category that has been defined"
                                                                    hidden="{{ $hidden }}">
                                                                    @foreach ($categories as $item)
                                                    ...
                                                    <div class="flex-fill">
                                                        <div class="mb-8 fv-row fv-plugins-icon-container">
                                                            <label for="status"
                                                                class="form-label">@lang('Status')</label>
                                                            <select name="status"
                                                                class=" form-control form-select-solid mb-2"
                                                                required="" id="status">
                                                        ...
                                                        @foreach ($template['lang_option'] as $lang)
                                                            ...
                                                            <li class="nav-item" role="presentation">
                                                                <button
                                                                    ...
                                                                    data-bs-target="#{{ $lang_code }}" type="button"
                                                                    role="tab" aria-controls="{{ $lang_code }}"
                                                                    aria-selected="true">
                                                        ...
                                                        @foreach ($template['lang_option'] as $lang)
                                                            ...
                                                            <div class="tab-pane fade @if ($loop->first) show active @endif"
                                                                ...
                                                                @foreach ($components as $component)
                                                                    @include(
                                                                        'components.backend.component-wrapper-v2',
                                                                        [
                                                                            'component' => $component,
                                                                ...
                                                                @if ($type['type'] == 'news' || $type['type'] == 'blog')
                                                                    ...
                                                                    <h3 class="text-dark fw-bolder mb-5">Banner
                                                                        Configuration</h3>
                                                                    ...
                                                                    <div class="d-flex align-items-start">
                                                                        <div class="nav flex-column nav-pills me-3"
                                                                            ...
                                                                            @foreach (['left', 'right', 'center', 'bottom'] as $location)
                                                                                <button
                                                                                    class="nav-link {{ $loop->first && $location == 'left' ? 'active' : '' }}"
                                                                        ...
                                                                        <div class="tab-content flex-grow-1"
                                                                            id="v-pills-tabContent-{{ $lang_code }}">
                                                                            @foreach (['left', 'right', 'center', 'bottom'] as $location)
                                                                                <div class="tab-pane fade {{ $loop->first && $location == 'left' ? 'show active' : '' }}"
                                                                                    ...
                                                                                    <div class="mb-5">
                                                                                        <label class="form-label">Banner
                                                                                            Group</label>
                                                                                        <select
                                                                                            name="banner_active[{{ $lang_code }}][{{ $location }}][group_id]"
                                                                                    ...
                                                                                    <div class="mt-3">
                                                                                        <button type="button"
                                                                                            class="btn btn-sm btn-light-danger clear-banner-btn">Clear
                                                                                            Banner Config in this
                                                                                            Position</button>
                                                                                    </div>
                                                                                </div>
                                                                            @endforeach
