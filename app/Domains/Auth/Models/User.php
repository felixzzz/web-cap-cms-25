...
class User extends Authenticatable implements MustVerifyEmail, TwoFactorAuthenticatable
{
    ...
    /*function to get all permission including children of permission*/
    public function userPermissions()
    {
        $permissions = $this->getAllPermissions();
        return $this->getChildrenPermission($permissions);
    }

    private function getChildrenPermission($permissions)
    {
        $allPermission = [];
        foreach ($permissions as $permission) {
            $allPermission[] = $permission->name;
            if ($permission->children?->count() > 0) {
                $allPermission = array_merge($allPermission,[...$this->getChildrenPermission($permission->children)]);
            }
        }
