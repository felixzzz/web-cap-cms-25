...
class DynamicController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('backend.dynamic.index', compact('templates'));
    }

    public function create(Request $request)
    {
        $template = $this->dynamic_templates()->where('name', $request->template)->first();
        if ($template) {
            $components = $template['components'];
            $post = new Post();
            $parents = Post::where('type', 'page')
                        ->where(function ($query) {
                            $query->where('pages_dynamic', 'yes')->orWhere('slug', 'sustainability');
                        })
                        ->pluck('id', 'title');
            return view('backend.dynamic.create', compact('template', 'components', 'post', 'parents', 'template'));
        }
        return back();
    }
    ...
    public function store(Request $request)
    {
        ...
        return to_route('admin.dynamic')->withFlashSuccess(__('Page was successfully created.'));
    }

    public function edit(Post $post)
    {
        $template = $this->dynamic_templates()->where('name', $post->template)->first();
        if ($template) {
            $components = $template['components'];
            ...
            $parents = Post::where('type', 'page')->where(function ($query) {
                            $query->where('pages_dynamic', 'yes')
                                ->orWhere('slug', 'sustainability');
                        })->where('id', '!=', $post->id)->pluck('id', 'title');
            return view('backend.dynamic.edit', compact('template', 'components', 'post', 'parents', 'location'))->withMeta($valueMeta);
        }
    ...
    public function update(Request $request, Post $post)
    {
        ...
        return redirect()->route('admin.dynamic')
            ->withFlashSuccess(__('Page was successfully updated.'));
    }
    ...
    public function delete(Post $post)
    {
        ...
        return redirect()->route('admin.dynamic')->withFlashSuccess(__('Page was deleted successfully.'));
    }
    ...
    public function trashed()
    {
        ...
        return view('backend.dynamic.deleted');
    }
    ...
    public function restore(int $deletedPostId)
    {
        ...
        return redirect()->route('admin.dynamic')->withFlashSuccess(
    ...
    public function destroy(int $deletedPostId)
    {
        ...
        return redirect()->route('admin.dynamic.deleted')->withFlashSuccess(
    ...
    public function show(Post $post)
    {
        ...
        $theTemplate = $this->dynamic_templates()->where('name', $post->template)->first();
        ...
        return view('backend.dynamic.show')
            ->withPost($post)
            ->withParent($parent)
            ->withType($type)
     ...
     */
    public function dynamic_templates(): ?\Illuminate\Support\Collection
    {
        $template = null;
